        ...
        renderItem={({ item }) => {
          return (
            <TouchableOpacity
              ...
              onPress={() =>
                router.push({
                  pathname: "/business-provider/view-quotes/panding-view/[id]",
                  params: { id: item?.quote?.id, price: item?.price_offered, bid_id: item?.id },
                })
              }
            >
              <View style={tw`flex-row items-center justify-between p-4 relative`}>
                ...
                <View style={tw`flex-row items-center gap-2`}>
                  <Image
                    source={{ uri: makeImage(item?.quote?.avatar) }}
...
export default PandingViewYourQuotes;
