...
import { useEditeBidditsMutation, useGetMyBiddingQuery } from "@/src/redux/categoryWiseQuotesApi/categoryHomeApi";
...
const EditeQuoteModel = ({
    setModalVisible,
    modalVisible,
    id,
}: StartQuoteModelProps) => {
    const [startQuote, { isLoading }] = useEditeBidditsMutation();



    const { data } = useGetMyBiddingQuery({ quote_id: id })
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (res?.status === true) {
                Alert.alert("Success", "Your bid has been update");
                        ...
                        <View style={tw`gap-4`}>
                            ...
                            <View style={tw`gap-2`}>
                                ...
                                >
                                    <TextInput
                                        ...
                                        placeholder={`${data?.data?.price_offered}`}
                            ...
                            <View style={tw`gap-2`}>
                                ...
                                <View style={tw`bg-input_bg_gray rounded-lg h-32`}>
                                    <TextInput
                                        style={tw`p-4 w-full h-32 text-base text-title_color rounded-xl `}
                                        placeholder={data?.data?.quote_outline || "Write your outline..."}
                                        multiline
                                        numberOfLines={6}
...
export default EditeQuoteModel;
