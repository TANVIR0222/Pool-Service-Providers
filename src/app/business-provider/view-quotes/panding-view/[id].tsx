...
import { useCancelBidsProviderMutation, useViewSingleBrowseCodeQuery } from "@/src/redux/myServicesApi/myServicesApi";
...
const PandingViewScreen = () => {
  const { id, price, bid_id } = useLocalSearchParams<{ id: string; price: string; bid_id: string }>();
  const { data, isLoading } = useViewSingleBrowseCodeQuery({ quote_id: id });

  const [bids_deleted] = useCancelBidsProviderMutation()
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await bids_deleted({ quote_id: bid_id }).unwrap()
      router.replace('/business-provider/(drawer)/(tabs)')
      ...
      >
        ...
        <View style={tw`flex-1 justify-between`}>
          ...
          <View style={tw`py-4`}>
            ...
            <Text style={tw`text-base text-title_color font-roboto-600 py-2`}>
              ...
              <View style={tw`flex-col gap-6`}>
                ...
                <View style={tw`flex-col gap-2`}>
                  ...
                  <View style={tw`flex-row  items-center justify-start gap-2`}>
                    ...
                    >
                      {`Wait for home owners accept your bid request.`}
                    </Text>
                  </View>
                </View>
              </View>
...
export default PandingViewScreen;
